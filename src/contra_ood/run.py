...
from model import RobertaForSequenceClassification, GPT2ForSequenceClassification, T5ForSequenceClassification, prepare_ood
...
def train(args, model, train_dataset, dev_dataset, test_dataset, benchmarks):
    ...
    def detect_ood():
        prepare_ood(model, dataloader=train_dataloader, is_train=True) # The paper originally finds the 1NN from the dev dataset, but I am changing that to train. This also affects Maha calculation.
...
def main(args):
    ...
    if args.model_class == 'roberta':
        ...
        # Match GPT2 config variables to RoBERTa (to manage common classification head)
        ...
        # Match T5 config variables to RoBERTa (to manage common classification head)
...
if __name__ == "__main__":
    ...
    args.sentence_embedding = 'average'  # 'average' 'cls'
    args.contrastive_loss = 'margin'  # 'supcon' 'margin'

    args.max_seq_length = 256
    args.do_train = False
    args.linear_probe = False
    args.tau = 0.3                   # Originally 0.3 - Temperature for SupCon
    args.contrastive_weight = 2.0    # Originally 2.0
