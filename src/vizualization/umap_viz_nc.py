...
def save_final_layer_embeddings(save_name, processed_dataset, data_util, model_name, is_id,
    ...
    if is_id:
        np.savez_compressed(f'{paths.temp_output_dir}/final_hidden_state_{save_name}_id.npz', final_hidden_state)
        np.savez_compressed(f'{paths.temp_output_dir}/labels_{save_name}_id.npz', labels)
    else:
        np.savez_compressed(f'{paths.temp_output_dir}/final_hidden_state_{save_name}_ood.npz', final_hidden_state)
        np.savez_compressed(f'{paths.temp_output_dir}/labels_{save_name}_ood.npz', labels)
...
def umap_visualization(x_dict, filename, metric='cosine', min_dist=0.1, n_neighbors=10, densmap=True):
    ...
    umap.plot.points(embedding, labels=y_test, color_key={'NewsCategory-ID': 'blue', 'NewsCategory-OOD': 'yellow'})
...
if __name__ == "__main__":
    id_dataset = 'news-category-modified'  # '20newsgroups' 'sst2' 'news-category-modified' 'clinc150'
    ood_datasets = ['news-category-modified']
    ...
    for epoch in range(10):
        ...
        # model_name = f'finetuned_models/ce/id_nc/roberta_base_news-category-modified_ce_epoch-{epoch}'
        # model_name = f'finetuned_models/tapt/id_nc/roberta_base_news-category-modified_tapt_epoch-{epoch}'
        # model_name = f'finetuned_models/supcon/id_nc/epoch_sweep/roberta_base_news-category-modified_contrastive_tau0.1_alpha2.0_epoch-{epoch}'

        id_dataset_tensors, id_data_util = process_dataset(id_dataset, is_id=True)
        save_final_layer_embeddings(save_name='NewsCategory', processed_dataset=id_dataset_tensors, data_util=id_data_util, is_id=True,
                                    get_cls_embedding=True, model_name=model_name,
                                    penultimate_layer=penultimate_layer, debug_mode=run_configs.debug_mode)

        for ood_dataset in ood_datasets:
            ood_dataset_tensors, ood_data_util = process_dataset(ood_dataset, is_id=False, split_idx=3)
            save_final_layer_embeddings(save_name='NewsCategory-ID', processed_dataset=ood_dataset_tensors, data_util=ood_data_util, is_id=False,
                                        get_cls_embedding=True, model_name=model_name, debug_mode=run_configs.debug_mode,
                                        penultimate_layer=penultimate_layer)
            ood_dataset_tensors, ood_data_util = process_dataset(ood_dataset, is_id=False, split_idx=1)
            save_final_layer_embeddings(save_name='NewsCategory-OOD', processed_dataset=ood_dataset_tensors,
                                        data_util=ood_data_util, is_id=False,
                                        get_cls_embedding=True, model_name=model_name,
                                        debug_mode=run_configs.debug_mode,
                                        penultimate_layer=penultimate_layer)

        get_umap_plots_from_embeddings(['NewsCategory', 'NewsCategory-ID', 'NewsCategory-OOD'], filename=f'nc_ce_epoch-{epoch}')
